const milestones = [
'<img id="asset-0" src="./milestone-images/4962.png">',
'<a-asset-item id="asset-1-obj" src="./milestone-images/2002.obj"></a-asset-item><a-asset-item id="asset-2-mtl" src="./milestone-images/2002.mtl"></a-asset-item>',
'<img id="asset-2" src="./milestone-images/Schwarzes Brett.png">',
'<img id="asset-3" src="./milestone-images/Treppen.png">',
'<img id="asset-4" src="./milestone-images/auge_02.png">',
'<img id="asset-5" src="./milestone-images/boule_02.png">',
'<a-asset-item id="asset-6-obj" src="./milestone-images/kleiderbuegel.obj"></a-asset-item><a-asset-item id="asset-6-mtl" src="./milestone-images/kleiderbuegel.mtl"></a-asset-item>',
'<img id="asset-7" src="./milestone-images/butt.png">',
'<a-asset-item id="asset-8-obj" src="./milestone-images/dart.obj"></a-asset-item><a-asset-item id="asset-8-mtl" src="./milestone-images/dart.mtl"></a-asset-item>',
'<a-asset-item id="asset-9-obj" src="./milestone-images/efeu.obj"></a-asset-item><a-asset-item id="asset-9-mtl" src="./milestone-images/efeu.mtl"></a-asset-item>',
'<img id="asset-10" src="./milestone-images/feuer_02.png">',
'<img id="asset-11" src="./milestone-images/frisbee.png">',
'<img id="asset-12-texture" src="./milestone-images/earth-texture.jpg">',
'<a-asset-item id="asset-13-obj" src="./milestone-images/1-250.obj"></a-asset-item><a-asset-item id="asset-13-mtl" src="./milestone-images/1-250.mtl"></a-asset-item>',
'<a-asset-item id="asset-14-obj" src="./milestone-images/tire.obj"></a-asset-item>',
'<img id="asset-15" src="./milestone-images/kant.png">',
'<img id="asset-16" src="./milestone-images/kruecke_04.png">',
'<img id="asset-17" src="./milestone-images/popcorn.png">',
'<a-asset-item id="asset-18-obj" src="./milestone-images/regenschirm.obj"></a-asset-item><a-asset-item id="asset-18-mtl" src="./milestone-images/regenschirm.mtl"></a-asset-item>',
'<img id="asset-19" src="./milestone-images/tapete.png">',
'<a-asset-item id="asset-20-obj" src="./milestone-images/castle.obj"></a-asset-item><a-asset-item id="asset-20-mtl" src="./milestone-images/castle.mtl"></a-asset-item>',
'<img id="asset-21" src="./milestone-images/yinyang.png">',
'<img id="asset-22" src="./milestone-images/zzz_02.png">',
'',
'<a-asset-item id="locked-obj" src="./milestone-images/locked.obj"></a-asset-item><a-asset-item id="locked-mtl" src="./milestone-images/locked.mtl"></a-asset-item>'
];

export default milestones;
